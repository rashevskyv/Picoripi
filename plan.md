...
## Етап 18: Скіл автоматичного оновлення вікі та README (update-wiki)

- [x] Розширити специфікацію скіла `update-wiki` у `.agents/skills/update-wiki/SKILL.md` та `.grok/skills/update-wiki/SKILL.md` чітким регламентом виявлення нових функцій, плагінних хуків, переміщень елементів UI та меню інтерфейсу.
- [x] Визначити обов'язкові кроки синхронізації: англійські сторінки `docs/wiki/*.md`, українські дзеркала `docs/wiki/uk/*.md` та оновлення розділів функціонала й навігації в `README.md`.
- [x] Застосувати оновлений скіл для актуалізації поточної вікі та `README.md` щодо нещодавніх змін (перенесення чекбокса «Needs review» під таблицю глосарію, кнопка «Wiki ↗», згортання панелей деталей, фільтри Mentions/Spoken, пакетний переклад i18n, скіл деплою).
- [x] Перевірити `ruff`, `git diff --check`, паралельні тести та актуальність графа знань.
