# --- START OF FILE plugins/zelda_bmg/tag_logic.py ---
import re
...
def _analyze_tags_for_issues_zbmg(processed_text: str, original_text: str) -> Tuple[str, str]:
    ...
    original_player_count = original_tags.count(PLAYER_TAG_BMG)
    processed_player_count = processed_tags.count(PLAYER_TAG_BMG)
    if original_player_count != processed_player_count:
        error_msg = (f"Player tag '{PLAYER_TAG_BMG}' count mismatch. Processed: {processed_player_count}, "
                     f"Original: {original_player_count}.")
        return TAG_STATUS_WARNING, error_msg
        
    original_color_tags = [tag for tag in original_tags if COLOR_TAG_PATTERN_BMG.fullmatch(tag)]
    processed_color_tags = [tag for tag in processed_tags if COLOR_TAG_PATTERN_BMG.fullmatch(tag)]
    if len(original_color_tags) != len(processed_color_tags):
         error_msg = (f"Color tag '{COLOR_TAG_PATTERN_BMG.pattern}' count mismatch. Processed: {len(processed_color_tags)}, "
                     f"Original: {len(original_color_tags)}.")
         return TAG_STATUS_WARNING, error_msg

    return TAG_STATUS_OK, ""

def process_segment_tags_aggressively_zbmg(
