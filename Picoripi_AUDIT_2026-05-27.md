...
### Оновлення: v0.2.167 (2026-06-08)
В рамках версії `v0.2.167` реалізовано:
- **Виправлення скидання дефолтного значення у MassWidthDialog**:
  - Виправлено баг, через який кнопка **Default** у діалоговому вікні групового встановлення ширини (`MassWidthDialog`) скидала ширину до порогу попередження (`line_width_warning_threshold_pixels`, наприклад, 410px), а не до максимального ліміту діалогу (`game_dialog_max_width_pixels`, наприклад, 435px), встановленого у налаштуваннях проекту.
  - Оновлено опис QLabel англійською мовою, який тепер чітко інформує про можливість скидання через введення `0` або `435` (максимальної ширини).
- **Уніфікація та посилення підсвітки Font та Width**:
  - Уніфіковано стиль підсвітки параметрів у `StringSettingsUpdater`. Обидва поля (`Font` та `Width`) при наявності кастомних значень (overrides) тепер мають ідентичний стиль `self.highlight_style`: жирну рамку `2px solid rgb(186, 85, 211)` та фон `rgba(186, 85, 211, 40)`.
  - Змінено спосіб застосування стилю до `QSpinBox`: тепер він призначається безпосередньо для всього віджета `QSpinBox` (а не його внутрішнього `lineEdit`), що дозволило зберегти нативні кнопки (стрілочки) на Windows і зробити рамку суцільною навколо всього поля.
...
### Оновлення: v0.2.169 (2026-06-08)
В рамках версії `v0.2.169` реалізовано:
- **Модальний прогрес-бар при скиданні фільтрів**:
  * Реалізовано прапорець `_load_fully_synchronously` для примусового повного синхронного завантаження великих блоків по 100 рядків із періодичним викликом `QApplication.processEvents()` для плавного оновлення прогресу від 0% до 100%.
  * Встановлено `setMinimumDuration(0)` та явні виклики `show()` / `raise_()` для `QProgressDialog`, що гарантує його миттєву появу та повне блокування інтерфейсу від дій користувача під час тривалого рендерингу.
  * Утримання вікна прогресу відкритим протягом усього життєвого циклу рендерингу та відновлення скролу/виділення для усунення візуального миготіння та стрибків скролу.
- **Покращення сумісності тестів**:
  * Додано динамічне визначення джерела даних `data_source` на випадок, якщо `data_store` є MagicMock (для сумісності з тестом збереження скролу).
  * Реалізовано автоматичне вимкнення чанкового рендерингу для MagicMock-документів у тестах для уникнення `TypeError: argument 1 has unexpected type 'MagicMock'` при роботі з `QTextCursor`.
  * Додано тестовий прапорець `_force_progress_for_testing` для ізольованого тестування викликів прогрес-діалогу в мок-середовищі.
  * Оновлено тести `test_UIUpdater_populate_strings_preserves_scrollbar` та `test_ListSelectionHandler_toggle_show_overrides_only_unchecked_same_string` відповідно до нових вимог синхронності та збереження стану скролу.
### Оновлення: v0.2.170 (2026-06-08)
В рамках версії `v0.2.170` реалізовано:
- **Виправлення поведінки Auto-fix для Twilight Princess (zelda_bmg)**:
  - Додано перевірку кодів переходу сторінок та пауз (наприклад, `{escape:0:0007000a}` або `{pause}`) у плагін `zelda_bmg` (методи `_check_short_line_zbmg` в `problem_analyzer.py` та `_fix_short_lines_zbmg` в `text_fixer.py`). Це запобігає некоректному злиттю коротких ліній діалогу та руйнуванню ручної верстки.
  - Оновлено загальну функцію `shift_split_sentences` в `utils/utils.py`. Тепер вона не видаляє порожні рядки, а розглядає їх як жорсткі межі сторінок. Також додано підтримку автоматичного доповнення поточної сторінки порожніми рядками перед початком речення, що починається з коду переходу сторінки.
  - Адаптовано тест `test_autofix_page_isolation` у `tests/test_spacing_rules.py` під нову правильну поведінку збереження порожніх рядків-роздільників.

### Оновлення: v0.2.171 (2026-06-08)
В рамках версії `v0.2.171` реалізовано:
- **Збереження стану фільтра «Hide Empty Strings»**:
  - Реалізовано збереження стану галочки «Hide Empty Strings» у файлі конфігурації `settings.json` та автоматичне відновлення прапорця при перезапуску програми.
  - Налаштовано мок-безпечне зчитування параметрів при роботі з `MagicMock` під час тестування.
- **Нова стійка консоль для запуску зовнішніх скриптів на Windows**:
  - Переведено запуск зовнішніх інструментів/скриптів на запуск через `cmd.exe /k` із прапором `CREATE_NEW_CONSOLE`. Це дозволяє відкривати нове видиме консольне вікно, яке залишається відкритим після завершення роботи скрипта для аналізу логів та виводу.
- **Оновлення тестів**:
  - Оновлено тести у `tests/test_settings_manager.py` для перевірки збереження та завантаження налаштування `hide_empty_strings`.

### Оновлення: v0.2.172 (2026-06-08)
В рамках версії `v0.2.172` реалізовано:
- **Делікатне підкреслення помилок у полі пошуку**:
  - Реалізовано кастомний `paintEvent` у `SearchLineEdit` (`components/search_panel.py`), який малює червону хвилясту лінію під словами з помилками за допомогою `QPainter`.
  - Динамічне визначення екранних координат літер виконано через бінарний пошук та метод `cursorPositionAt`. Це працює коректно при будь-якому горизонтальному прокручуванні тексту.
  - Повністю виключено зміну StyleSheet для `QLineEdit`, що усунуло проблему перетворення контекстного меню `QMenu` на повністю червоне.
  - Створено модульні тести у `tests/test_ui/test_search_panel_spellcheck.py`.
- **Автоматичне копіювання з прогресом у pack_iso.bat**:
  - Додано перевірку флеш-накопичувачів (виключено системні диски `C, D, E, G`) за наявністю папки `dusk`.
  - У разі виявлення накопичувача копіювання образу ISO та збереження `.gci` відбувається за допомогою `robocopy` з відображенням прогресу копіювання у відсотках у реальному часі.
