...
def test_resolve_glossary_path(tmp_path):
    """The glossary belongs to the project: resolved only in the project dir.

    Plugin-level and global copies are deliberately ignored — see
    tests/test_handlers/test_translation/test_glossary_path_resolution.py for the
    full rule set.
    """
    import os
    import time
    ...
    mock_mw = MagicMock()
    mock_mw.project_manager.project_dir = str(project_dir)
    gpm = GlossaryPromptManager(mock_mw, MagicMock(), MagicMock())

    # Nothing yet: the path points at the file that will be created on first write.
    assert gpm._resolve_glossary_path("test_plugin") == project_dir / "glossary.json"

    # An existing project glossary is used.
    project_json = project_dir / "glossary.json"
    project_json.write_text("[]", encoding="utf-8")
    assert gpm._resolve_glossary_path("test_plugin") == project_json

    # With both formats present, the newer one wins.
    project_md = project_dir / "glossary.md"
    project_md.write_text("markdown", encoding="utf-8")
    os.utime(project_json, (time.time() - 100, time.time() - 100))
    os.utime(project_md, (time.time(), time.time()))
    assert gpm._resolve_glossary_path("test_plugin") == project_md

    # Without an open project there is no glossary at all.
    mock_mw.project_manager = None
    assert gpm._resolve_glossary_path("test_plugin") is None
