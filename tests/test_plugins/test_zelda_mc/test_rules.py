...
def test_GameRules_get_display_name(rules):
    ...
    assert "Zelda" in name or "Minish" in name

def test_GameRules_get_default_tag_mappings(rules):
    mappings = rules.get_default_tag_mappings()
    assert isinstance(mappings, dict)

def test_GameRules_get_tag_checker_handler(rules):
    handler = rules.get_tag_checker_handler()
...
def test_GameRules_get_color_marker_definitions(rules):
    defs = rules.get_color_marker_definitions()
    assert isinstance(defs, dict)

def test_GameRules_get_short_problem_name(rules):
    from plugins.zelda_mc.config import PROBLEM_WIDTH_EXCEEDED, PROBLEM_SHORT_LINE
    assert rules.get_short_problem_name(PROBLEM_WIDTH_EXCEEDED) == "Width"
    assert rules.get_short_problem_name(PROBLEM_SHORT_LINE) == "Short"
    assert rules.get_short_problem_name("unknown_problem") == "unknown_problem"

def test_GameRules_get_plugin_actions(rules):
    actions = rules.get_plugin_actions()
    assert isinstance(actions, list)
    action_names = [a['name'] for a in actions]
    assert 'check_tags_mismatch' in action_names
...
def test_GameRules_analyze_subline(rules):
    ...
    from plugins.zelda_mc.config import PROBLEM_WIDTH_EXCEEDED
...
def test_GameRules_get_base_game_rules_class(rules):
    cls = rules.get_base_game_rules_class()
    assert cls is BaseGameRules
