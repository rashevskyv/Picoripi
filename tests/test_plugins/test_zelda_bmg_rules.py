...
def test_autofix_tab_preserves_text_order():
    rules = GameRules()
    setup_test_mappings(rules)

    text = "Line 2 {tab} text inside."
    allowed = {"ZBMG_SHORT_LINE", "ZBMG_WIDTH_EXCEEDED"}

    fixed, changed = rules.autofix_data_string(text, {}, 10000, allowed_problems=allowed)
    assert changed is True

    # Text before {tab} must remain before it, and text after must follow it.
    # The output should split into two lines:
    # "Line 2"
    # "{escape:6:000b}text inside."
    lines = fixed.split('\n')
    assert len(lines) == 2
    assert lines[0] == "Line 2"
    assert lines[1] == "{escape:6:000b}text inside."

    # Test double tab normalization (should not create empty lines)
    text_double = "A {tab}{tab} B"
    fixed_double, changed_double = rules.autofix_data_string(text_double, {}, 10000, allowed_problems=allowed)
    assert changed_double is True
    lines_double = fixed_double.split('\n')
