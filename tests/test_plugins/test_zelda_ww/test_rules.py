...
def test_GameRules_get_display_name(rules):
    ...
    assert "Zelda" in name or "Wind Waker" in name
...
def test_GameRules_get_short_problem_name(rules):
    from plugins.zelda_ww.config import PROBLEM_WIDTH_EXCEEDED, PROBLEM_SHORT_LINE
...
def test_GameRules_get_enter_char(rules):
    assert rules.get_enter_char() == '\n'
    
def test_GameRules_get_editor_page_size(rules):
    assert rules.get_editor_page_size() == 1
