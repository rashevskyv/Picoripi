...
def test_save_speaker_updates_virtual_speaker_and_selects_string(qapp, mock_mw, mock_project):
    """Test that when in virtual speaker mode, changing speaker retains the current speaker folder and switches focus to a sibling string."""
    mock_mw.project_manager = MagicMock()
    mock_mw.project_manager.project = mock_project
    mock_mw.block_to_project_file_map = {0: 0}
    ...
    # Rebuilt Hero item will only have (0,1) since (0,0) moved to Villain
    ...
    rebuilt_hero_item.setData(0, int(Qt.ItemDataRole.UserRole + 13), [(0, 1)]) # sibling (0,1) is at index 0
    ...
    # Mock string selection call
    handler.string_selected_from_preview = MagicMock()
    
    # Use patch for QTimer to run singleShot tasks synchronously
    with patch('PyQt6.QtCore.QTimer.singleShot', side_effect=lambda ms, func: func()):
        # Change speaker of current string (0,0) to "Villain"
        handler.save_speaker_for_current_string("Villain")
    
    # 1. Verify speaker name remained "Hero" (we retained folder selection)
    assert mock_mw.data_store.current_speaker_name == "Hero"
    
    # 2. Verify targets were cleared by block_selected
    assert handler._target_block_idx is None
    assert handler._target_string_idx is None
    
    # 3. Verify populate_blocks was called with Hero (the retained folder)
    assert mock_mw.ui_updater.block_list_updater.populate_blocks.called
    
    # 4. Verify string_selected_from_preview was called with index 0 (corresponding to sibling (0,1))
