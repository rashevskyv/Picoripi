...
> **Остання версія проекту:** v0.3.101-dev
...
- **AUD-U1. Оптимізація та реорганізація інтерфейсу глосарію (v0.3.099-dev, 2026-09-07).**
  - Об'єднано рядок оригіналу та перекладу: вихідний термін завжди доступний для перегляду й копіювання зліва у `QPlainTextEdit` (read-only), а поле редагування перекладу — справа з компактною кнопкою підтвердження.
  - Додано кнопку переходу до зовнішньої вікі гри («Wiki ↗») через контракт плагінів `BaseGameRules.get_external_reference_url(term)` (реалізовано для Zelda Wiki в плагінах Zelda).
  - Секції «Опис», «Нотатки AI» та «Входження» отримали механізм згортання/розгортання `[▼]/[▶]` із динамічним зменшенням висоти панелей для максимальної економії простору.
  - Чекбокс «Profiled via AI» перенесено у заголовок панелі опису з детальним тултіпом (пояснює стан метаданих профілювання мовлення персонажа).
  - У панель входжень додано незалежні фільтри «Mentions» та «Spoken» для зручного відокремлення прямих згадок терміна в тексті від реплік відповідного персонажа.
