...
**Language** показує лише **English** і **Українська**. Зміна записує `ui_language` і просить перезапуск.

Інших мов інтерфейсу в цьому меню ще немає. Їх заповнює `tools/i18n-translate/run.bat` перед багатомовним деплоєм, після чого код можна додати в `SHIPPED_UI_LANGUAGES` у `core/i18n.py`.
